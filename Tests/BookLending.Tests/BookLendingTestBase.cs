...
namespace BookLending.Tests
{
    public abstract class BookLendingTestBase : AbpIntegratedTestBase<BookLendingTestModule>
    {
        ...
        protected void UsingDbContext(Action<BookLendingDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<BookLendingDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<BookLendingDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<BookLendingDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<BookLendingDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BookLendingDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<BookLendingDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BookLendingDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<BookLendingDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<BookLendingDbContext, Task<T>> func)
        {
            T result;
